...
atop <- rio::import(paste0(here::here(), "/inst/extdata/ATOP/atop5_0sy.csv"))
```
...
atop_sub <- atop %>%
  dplyr::select(state,
                ...
                atopally,
                number) %>%
  dplyr::rename(country_code_cow = state,
                atop_atopally = atopally,
                atop_atopcount = number) %>%
  dplyr::mutate(atop_atopally = as.numeric(atop_atopally),
                atop_atopcount = as.numeric(atop_atopcount))
...
geog_covariates$atop_atopally[is.na(geog_covariates$atop_atopally)] <- 0
geog_covariates$atop_atopcount[is.na(geog_covariates$atop_atopcount)] <- 0
```
